...
export default class NewListingScreen extends Component<Props> {
  ...
  async uploadListing() {
    var time = parseInt(this.state.remaining_time) * 60;
    jsonObject = {
      ...
      <ScrollView style={styles.container}>
        <View style={styles.imageContainer}>
          <ImageBackground
            ...
            <ImagePickerComponent
              onChange={imageData => this.onChangeImage(imageData)}
            />
          </ImageBackground>
        </View>
        ...
        <View style={styles.fieldsAreaContainer}>
          <View style={styles.fieldContainer}>
            <Label_Field2
              label={"Title"}
              onChange={text => this.changeState(text, "title")}
              placeholder={"e.g. used watch"}
              textInputHeight={50}
            />
          </View>
          <View style={styles.fieldContainer}>
            <Label_Field2
              ...
              onChange={text => this.changeState(text, "original_price")}
              placeholder={"e.g. 50"}
              textInputHeight={50}
            />
          </View>
          <View style={styles.fieldContainer}>
            <Label_Field2
              label={"Min Price"}
              onChange={text => this.changeState(text, "min_price")}
              placeholder={"e.g. 5"}
              textInputHeight={50}
            />
          </View>
          <View style={styles.fieldContainer}>
            <Label_Field2
              ...
              onChange={text => this.changeState(text, "remaining_time")}
              placeholder={"e.g. 300 minutes"}
              textInputHeight={50}
            />
          </View>
          <View style={styles.fieldContainer}>
            <Label_Field2
              ...
              onChange={text => this.changeState(text, "points_required")}
              placeholder={"e.g. 10"}
              textInputHeight={50}
            />
          </View>
          <View style={styles.fieldContainer}>
            <Label_Field2
              ...
              onChange={text => this.changeState(text, "description")}
              placeholder={"e.g. Product Value"}
              textInputHeight={50}
            />
          </View>
          <View style={styles.pickupButtonContainer}>
            {this.state.uploadProgress != "-2" ? (
              <Text style={styles.loadingText}>
                Upload Completed: {this.state.uploadProgress}%
              </Text>
            ) : null}
            <Button onPress={() => this.uploadListing()} title='Share' />
